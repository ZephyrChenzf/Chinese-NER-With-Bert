...
def main(args):
    ...
    model=BertChineseNER.from_pretrained(args.bert_model,num_labels = num_labels)
    if args.fp16:
        model.half()
    model.to(device)
    ...
    if args.do_train:
        ...
        print("  Num examples = %d"%len(train_x))
        print("  Batch size = %d"% args.train_batch_size)
        print("  Num steps = %d"% num_train_optimization_steps)
        all_input_ids = torch.tensor(train_x, dtype=torch.long)
        all_input_mask = torch.tensor(train_mask, dtype=torch.long)
        all_label_ids = torch.tensor(train_y, dtype=torch.long)
